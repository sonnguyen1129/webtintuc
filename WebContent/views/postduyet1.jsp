...
<body id = "body">
	...
	<%	
		if(request.getAttribute("listDuyet") != null){
			List<Post> listDuyet1 = (List<Post>) request.getAttribute("listDuyet");
			for(Post post: listDuyet1){
			...
			request.setAttribute("listDuyet1", listDuyet1);
		}
	...
	%>
		...
		<h2> ${message} </h2>
		<div>
	       	<c:forEach items="${listDuyet1}" var = "post">
		          ...
		          <div>
		            ...
		            <p style="font-size: 11px;">Ngày duyệt: ${post.ngayDuyet}</p>
		            <p style="font-size: 11px;">${post.view} lượt xem</p>
		          </div>
		        </div>
	       	</c:forEach>
	  	</div>
